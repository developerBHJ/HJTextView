...
@available(iOS 9.0, *)
@IBDesignable
open class HJCustomTextView: UITextView {
    ...
    private var lineView: UIView & HJInputViewProtocol = HJLineLeftAnimateView()
    ...
    private var shiftPlaceholderTitle: Bool = false {
        didSet {
            ...
            if shiftPlaceholderTitle {
                ...
                placeHolderLabel.textColor = HJCustomTextView.systemPlaceHolderColor
                placeHolderLabel.hj_TextLayout
                    .setTop(anchor: self.lineContainerView.topAnchor, type: .equal(constant: 0))
    ...
    private var placeHolderLabel: UILabel = {
        ...
        label.textColor = HJCustomTextView.systemPlaceHolderColor
    ...
    public var titleFont: UIFont? {
        didSet {
            ...
            self.layoutIfNeeded()
            self.layoutChanged?()
            (self.delegate as? HJTextViewdProtocol)?.textLayoutChanged(text: self)
        }
    }
    ...
    override open var intrinsicContentSize: CGSize {
        ...
        let textH = max(textHeight,placeHeight) + 2*HJCustomTextView.defaultMargin
        ...
        self.textContainerInset = UIEdgeInsets(top: realTopHeight+HJCustomTextView.defaultMargin,
                                               left: 0,
                                               bottom: realBottomHeight+HJCustomTextView.defaultMargin,
                                               right: 0)
        return size
    }
    @IBInspectable
...
extension HJCustomTextView {
    ...
    private func updatePlaceHolderMargin() {
        ...
        case .border, .boderWith(_):
            ...
            lineContainerView.hj_TextLayout[.width]?.constant = 2*x
            placeHolderLabel.hj_TextLayout[.width]?.constant = -2*x
        case .line:
            lineContainerView.hj_TextLayout[.width]?.constant = 0
            placeHolderLabel.hj_TextLayout[.width]?.constant = 0
        }
    }
