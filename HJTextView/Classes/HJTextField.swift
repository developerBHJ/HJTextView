...
@IBDesignable
@available(iOS 9.0, *)
open class HJTextField: UITextField {
    ...
    var lineView: UIView & HJInputViewProtocol = HJLineLeftAnimateView()
    ...
    private var shiftPlaceholderTitle: Bool = false {
        didSet {
            ...
            if shiftPlaceholderTitle {
                ...
                placeHolderLabel.hj_TextLayout.setTop(anchor: self.topAnchor, type: .equal(constant: 0))
                    .setHeight(type: .equalTo(anchor: self.titleLabel.heightAnchor, multiplier: 1, constant: 0))
            } else {
                placeHolderLabel.textColor = HJTextField.systemPlaceHolderColor
                placeHolderLabel.hj_TextLayout
    ...
    private(set) public var placeHolderLabel: UILabel = {
        let label = UILabel()
        label.textColor = HJTextField.systemPlaceHolderColor
    ...
    public var titleFont: UIFont? {
        didSet {
            ...
            self.layoutIfNeeded()
            self.layoutChanged?()
            (self.delegate as? HJTextFieldProtocol)?.textLayoutChanged(text: self)
        }
    }
    ...
    public var lineType: LineType = .left {
        didSet {
            ...
            switch lineType {
            case .left:
                lineView = HJLineLeftAnimateView()
            case .right:
                lineView = HJLineRightAnimateView()
            case .center:
                lineView = HJLineCenterView()
            }
            lineView.duration = duration
    ...
    public var inputViewStyle: HJInputViewStyle {
        set {
    ...
    public var title: String?  {
        set {
            ...
            titleLabel.hj_TextLayout[.bottom]?.constant = -realTopMargin
            self.invalidateIntrinsicContentSize()
            self.layoutIfNeeded()
            self.layoutChanged?()
            (self.delegate as? HJTextFieldProtocol)?.textLayoutChanged(text: self)
    ...
    public var errorTitle: String? = nil {
        didSet {
            ...
            if errorTitle?.count == 0 {
                ...
                self.lineView.lineColor = errorColor
                self.textColor = errorColor
            }
            self.errorLabel.sizeToFit()
            errorLabel.hj_TextLayout[.top]?.constant = realBottomMargin
            self.invalidateIntrinsicContentSize()
            self.layoutIfNeeded()
            self.layoutChanged?()
            (self.delegate as? HJTextFieldProtocol)?.textLayoutChanged(text: self)
    ...
    @IBInspectable
    public var editLineWidth:Float = 1 {
        didSet {
    ...
    override open var intrinsicContentSize: CGSize {
        ...
        lineContainerView.hj_TextLayout[.height]?.constant = size.height + 8
    ...
    open func setup() {
        ...
        self.addTarget(self, action: #selector(HJTextField.beginEdit), for: .editingDidBegin)
        self.addTarget(self, action: #selector(HJTextField.endEdit), for: .editingDidEnd)
        self.addTarget(self, action: #selector(HJTextField.valueChange), for: .editingChanged)
        titleLabel.setContentHuggingPriority(.init(750), for: .vertical)
        titleLabel.hj_TextLayout
        ...
        lineContainerView.hj_TextLayout
...
extension HJTextField {
    ...
    private func updatePlaceHolderMargin() {
        ...
        case .border, .boderWith(_):
            ...
            placeHolderLabel.hj_TextLayout[.leading]?.constant = x
            placeHolderLabel.hj_TextLayout[.trailing]?.constant = x*parameter
            
        case .line:
            placeHolderLabel.hj_TextLayout[.leading]?.constant = 0
            placeHolderLabel.hj_TextLayout[.trailing]?.constant = 0
        }
    }
